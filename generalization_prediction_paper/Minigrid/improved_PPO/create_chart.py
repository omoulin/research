...
def generate_models():
   ...
   for i in range(50000,50000+nb_agent):
      tmp_bs=np.load('../../../experiments/Training_upgraded/baselines_training_gen_'+str(i)+'.npy')
      tmp_up=np.load('../../../experiments/Training_upgraded/upgraded_training_gen_'+str(coef)+'_'+str(i)+'.npy')
      for j in range(0,20):
   ...
   plt.legend()
   plt.title('Minigrid - Upgraded PPO - Gen. loss coef '+str(coef))
   plt.savefig('../../../experiments/Training_upgraded/Minigrid_UpgradedPPOvsBaseline_'+str(coef)+'.pdf')
   plt.clf()
