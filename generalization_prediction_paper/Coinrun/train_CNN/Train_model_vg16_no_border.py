...
cpt =0
for i in ra:
   weights_npy_EXT_1=np.load('../../../experiments/Coinrun/weights/Weights_EXT_1_'+str(i)+'.npy')
   weights_npy_EXT_2=np.load('../../../experiments/Coinrun/weights/Weights_EXT_2_'+str(i)+'.npy')
   weights_npy_ACT=np.load('../../../experiments/Coinrun/weights/Weights_ACT_'+str(i)+'.npy')
   ...
   #print(image.shape)
   data.append(image)
   gen=np.load('../../../experiments/Coinrun/generalization/Average_reward'+str(i)+'.npy')
   labels.append(gen[0])
...
sind = np.random.choice(len(data), size=150, replace=False)
test_data = data[sind]
test_labels = labels[sind]
train_data_round = np.delete(data, sind, axis=0)
train_labels_round = np.delete(labels, sind, axis=0)
sind = np.random.choice(len(train_data_round), size=150, replace=False)
validation_data = train_data_round[sind]
validation_labels = train_labels_round[sind]
train_data_round = np.delete(train_data_round, sind, axis=0)
train_labels_round = np.delete(train_labels_round, sind, axis=0)

for split in range(0,200):
   print("Running Data Chunk - ",str(split))
#   list_elements=150+random.randint(0,2049)
...
#      train_labels_round=np.append(train_labels_round,[labels[list_elements]],axis=0)
   ...
   chart_prediction = np.zeros(150)
   for i in range(0,150):
      chart_prediction[i]=pred[i]
   chart_labels = np.zeros(150)
   for i in range(0,150):
   ...
   plt.legend()
   plt.savefig('Predict_simple_reward_'+str(split)+'.pdf')
   plt.clf()
   model.save_weights('../../../experiments/Coinrun/CNN_model/predict_vg16_no_border.model')
